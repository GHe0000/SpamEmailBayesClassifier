...
- *计算条件概率*：通过如下式子计算当标签为 $l_m$ 时，词 $v_n$ 出现的概率（同样使用 Laplace 平滑）：

$ P(v_n|l_m) = (omega_n + s) / (sum_(omega_n in Omega) omega_n + s |V|) $

其中 $s$ 是 Laplace 平滑的超参数（在代码中设为 1），$|V|$ 是词汇表 $V$ 的大小（即 $n$）.

上述过程中得到的词汇表 $V$、先验概率 $P = {P(l_m)|l_m in L}$、条件概率 $Q$，以及词汇的 IDF 值 $I$ 即分类器的全部参数. 其中:

$ Q = {P(v_n|l_m)|v_n in V,l_m in L} $
...
$ log(P(l_m|d)) = log(P(l_m)) + sum_(omega'_n in Omega') omega'_n dot.c log(P(v_n|l_m)), quad v_n in V $

选取概率最大的标签作为预测结果.

#mark[这里也有一个小问题，为什么使用对数进行计算？为什么是 $omega'_n dot.c log(P(v_n|l_m))$ 的形式？
...
对于原始的 Bayes 分类器，其计算类别概率的公式为：

$ P(l_m|d) = P(l_m) product_(v_n in V) P(v_n|l_m)^(k_n) $

其中 $k_n$ 为词 $v_n$ 在 $d$ 中出现的次数.

现在我们将 $k_n$ 使用 TF-IDF 进行替换（TF-IDF 相当于给词频乘上一个修正系数），即可得到（一些常数项省略，最终为正比于的形式）: 

$ P(l_m|d) prop P(l_m) product_(v_n in V) P(v_n|l_m)^(omega'_n) $

上式取对数，即可得到原计算公式.

从另一方面，经过 log 变换后使得概率的计算具有#highlight[线性性]. 不难注意到：
$ P_m: v_n mapsto log(P(v_n|l_m)) $
满足：
$ P_m (v_n) + P_m (v_m) = P_m (v_n + v_m) $
而不取 log 则没有上面的线性性质.
]

#remark[从另一个方面来看使用对数进行计算同时也防止了浮点下溢，尤其是在长文档中.]
...
在 500 个测试集邮件中，准确率为 0.9433771486349848
...
在 500 个测试集邮件中，精确率为 0.9519832985386222.
...
在 500 个测试集邮件中，召回率为 0.9325153374233128
.
...
在 500 个测试集邮件中，F1 值为 0.9421487603305785.
...
当我们通过训练得到 $Q = {P(v_n|l_m)|v_n in V, l_m in L}$ 后，我们可以统计每个类别中出现概率最大的前 10 个词.
...
正常邮件的邮件内容大多比较随机，因此出现概率最大的词也比较随机，看不出什么明显的特征.
...
垃圾邮件的邮件内容大多比较相似，从上述内容来看，垃圾邮件一般是一些公司的广告，且一般会带有网址.

== 最具有垃圾邮件特征的邮件

由于垃圾邮件大多比较相似，因此我们可以从测试的 500 个邮件中计算是垃圾邮件的概率与是正常邮件的概率相差最大的邮件，作为最具有垃圾邮件特征的邮件.

在测试的 500 个邮件中，最具有垃圾邮件特征的邮件的词列表如下：

#codeBlock[
```
['公司', '负责人', '经理', '财务', '您好', '深圳市', '创达', '实业', '有限公司', '我司', '实力雄厚', '有着', '社会关系', '部分', '外省市', '票据', '进项', '较多现', '完成', '每月', '销售额', '每月', '一部分', '增值税', '电脑', '发票', '左右', '普通商品', '销售税', '发票', '国税', '地税', '广告', '运输', '服务', '左右', '优惠', '代开', '合作', '点数', '数量', '额度', '大小', '商讨', '优惠', '点数', '公司', '郑重', '承诺', '所用', '绝对', '真票', '希望', '能够', '机会', '合作', '如贵司', '发票', '真伪', '方面', '疑虑', '担心', '上网', '查证', '我司', '直接', '贵司', '税务局', '抵扣', '核对', '合作', '一次', '永久', '朋友', '信息', '长期有效', '进一步', '洽商', '详情请', '小时', '业务', '热线', '联系人', '赵铭', '顺祝', '商祺', '优惠', '开发票']
```
]
...
#codeBlock[
```
Message-ID: <200508141110.j7E3AQWj006740@spam-gw.ccert.edu.cn>
From: =?GB2312?B?08W73bT6v6q3osax?= <ye@12.com>
Subject: =?gb2312?B?08W73bT6v6q3osax?=
To: guan@ccert.edu.cn
Content-Type: text/plain;charset="GB2312"
Reply-To: ye@163.com
Date: Tue, 16 Aug 2005 05:49:51 +0800
X-Priority: 3
X-Mailer: Microsoft Outlook Express 6.00.2800.1106

贵公司负责人(经理/财务）您好： 
    我是深圳市创达实业有限公司的。我司实力雄厚，有着良
好的社会关系。也有部分外省市的票据，因进项较多现完成不
了每月销售额度。每月有一部分增值税电脑发票（6%左右）和
普通商品销售税发票国税、地税（广告、运输、服务2%左右）
优惠代开或合作，点数较低。还可以根据所做数量额度的大小
来商讨优惠的点数。
    本公司郑重承诺所用绝对是真票！更希望能够有机会与贵
司合作！
　　
    如贵司在发票的真伪方面有任何疑虑或担心，可上网查证
或我司直接与贵司去税务局抵扣核对。 
